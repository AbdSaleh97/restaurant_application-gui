...
def order_summary(pizza_quantity, pizza_size, burger_quantity, burger_size, soft_drink_quantity, extra_cheeses, extra_ketchup):
    total = calculate_total_price(
        pizza_quantity.get(), pizza_size.get(), burger_quantity.get(),
        burger_size.get(), soft_drink_quantity.get(), extra_cheeses.get(), extra_ketchup.get()
    )
...
Total Price: {total}'''
    with open("recipt.txt","w") as file:
        file.write(message)
...
if __name__ == "__main__":
    ...
    process_button = ttk.Button(frame, text="Submit", command=lambda: order_summary(
        pizza_quantity_var, pizza_size_var, burger_quantity_var, burger_size_var,
        soft_drinks_quantity_var, extra_cheese_var, extra_ketchup_var))
